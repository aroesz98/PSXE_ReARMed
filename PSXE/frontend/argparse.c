...
argparse_error(struct argparse *self, const struct argparse_option *opt,
               const char *reason, int32_t flags)
{
    (void)self;
    if (flags & OPT_LONG)
    {
                  ...
                  int32_t flags)
{
    ...
    case ARGPARSE_OPT_BOOLEAN:
        if (flags & OPT_UNSET)
        {
            *(int32_t *)opt->value = *(int32_t *)opt->value - 1;
        }
        else
        {
            *(int32_t *)opt->value = *(int32_t *)opt->value + 1;
        }
        if (*(int32_t *)opt->value < 0)
        {
            *(int32_t *)opt->value = 0;
        }
        break;
    ...
    case ARGPARSE_OPT_INTEGER:
        ...
        if (self->optvalue)
        {
            *(int32_t *)opt->value = strtol(self->optvalue, (char **)&s, 0);
            self->optvalue = NULL;
        }
        else if (self->argc > 1)
        {
            self->argc--;
            *(int32_t *)opt->value = strtol(*++self->argv, (char **)&s, 0);
        }
        else
        {
            argparse_error(self, opt, "requires a value", flags);
...
argparse_init(struct argparse *self, struct argparse_option *options,
              const char *const *usages, int32_t flags)
{
...
int32_t
argparse_parse(struct argparse *self, int32_t argc, const char **argv)
{
    self->argc = argc - 1;
    self->argv = argv + 1;
    self->out = argv;
