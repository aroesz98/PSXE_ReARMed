...
uint32_t psx_dma_read32(psx_dma_t *dma, uint32_t offset)
{
    if (offset < 0x70)
    {
        int32_t channel = (offset >> 4) & 0x7;
        int32_t reg = (offset >> 2) & 0x3;
        ...
        log_debug("DMA channel %u register %u (%08x) read %08x", channel, reg, PSX_DMAR_BEGIN + offset, cr);
    ...
    else
    {
        ...
        {
        case 0x70:
            log_debug("DMA control read %08x", dma->dpcr);
            return dma->dpcr;
        case 0x74:
            log_debug("DMA irqc    read %08x", dma->dicr);
            return dma->dicr;
...
void psx_dma_write32(psx_dma_t *dma, uint32_t offset, uint32_t value)
{
    ...
    else
    {
        ...
        {
        case 0x70:
            log_debug("DMA control write %08x", value);
            dma->dpcr = value;
            break;
        case 0x74:
        {
...
void psx_dma_do_mdec_in(psx_dma_t *dma)
{
    ...
    uint32_t size = BCR_SIZE(mdec_in) * BCR_BCNT(mdec_in);

    int32_t step = CHCR_STEP(mdec_in) ? -4 : 4;
