...
int32_t spu_handle_write(psx_spu_t *spu, uint32_t offset, uint32_t value)
{
    switch (offset)
    {
    case SPUR_KONL:
...
void __attribute__((section(".ramfunc.$SRAM_DTC"))) psx_spu_write32(psx_spu_t *spu, uint32_t offset, uint32_t value)
{
...
void __attribute__((section(".ramfunc.$SRAM_DTC"))) psx_spu_write16(psx_spu_t *spu, uint32_t offset, uint16_t value)
{
    // Handle special cases first
    if (spu_handle_write(spu, offset, value))
        return;
...
{
    PRINTF("Unhandled 8-bit SPU write at offset %08lx (%02x)\r\n", offset, value);
}
