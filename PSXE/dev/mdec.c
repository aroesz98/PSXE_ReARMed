...
// JPEG decode function adapted from sd_jpeg.c for MDEC motion JPEG decoding
                         ...
                         size_t output_width, size_t output_height, int32_t output_depth)
{
    ...
    // Allocate aligned buffer for JPEG data (similar to sd_jpeg.c)
    jpg_buffer = (uint8_t *)malloc(jpeg_size + 64);
    if (jpg_buffer == NULL)
    {
        log_error("MDEC JPEG: memory allocation error for %zu bytes", jpeg_size);
        ret = -1;
        goto cleanup;
    }

    // Align buffer to cache line if needed (from sd_jpeg.c pattern)
    jpg_buffer_aligned = (uint8_t *)(((uintptr_t)jpg_buffer + 31) & ~31);
    memcpy(jpg_buffer_aligned, jpeg_data, jpeg_size);

    // Step 3: Set source for decompression (memory source)
    jpeg_mem_src(&cinfo, jpg_buffer_aligned, jpeg_size);

    // Step 4: read image parameters with jpeg_read_header()
    if (jpeg_read_header(&cinfo, TRUE) != JPEG_HEADER_OK)
    {
        log_error("MDEC JPEG: Failed to read JPEG header");
        ret = -1;
        goto cleanup;
    }

    // Step 5: set parameters for decompression (from sd_jpeg.c)
    cinfo.dct_method = JDCT_FLOAT;

    // Resize to fit the output dimensions (scaling logic from sd_jpeg.c)
    if ((cinfo.image_width * output_height) > (cinfo.image_height * output_width))
    {
        cinfo.scale_num = output_width * 8 / cinfo.image_width;
    }
    else
    {
        cinfo.scale_num = output_height * 8 / cinfo.image_height;
    }
    ...
    // Calculate stride based on output depth
    if (output_depth == 3)
    {
        row_stride = output_width * 2; // 15-bit RGB (2 bytes per pixel)
    }
    else
    {
        row_stride = output_width * 3; // 24-bit RGB (3 bytes per pixel)
    }

    // Place the output image to the center of the buffer (from sd_jpeg.c)
    buffer = output_buffer;
    buffer += row_stride * ((output_height - cinfo.output_height) / 2);

    if (output_depth == 3)
    {
        buffer += 2 * ((output_width - cinfo.output_width) / 2); // 2 bytes per pixel for 15-bit
    }
    else
    {
        buffer += 3 * ((output_width - cinfo.output_width) / 2); // 3 bytes per pixel for 24-bit
    }

    // Step 7: Decompress scanlines
    ...
    log_info("MDEC JPEG: Successfully decoded %ux%u image to %zux%zu output",
             cinfo.image_width, cinfo.image_height, output_width, output_height);
...
void mdec_decode_macroblock(psx_mdec_t *mdec)
{
    ...
    else
    {
        ...
        unsigned long block_size = (mdec->output_depth == 3) ? 512 : 768;
        unsigned long bytes_processed = 0;
        unsigned long estimated_blocks = (mdec->input_size / 8) + 1; // Conservative estimate
        unsigned long max_output_size = estimated_blocks * block_size;
        const unsigned long MAX_MDEC_OUTPUT = 16 * 1024 * 1024;
        int32_t block_count = 0;

        // Check if input looks like JPEG data (starts with JPEG SOI marker 0xFFD8)
        if (mdec->input_size >= 2 && ((uint8_t *)mdec->input)[0] == 0xFF && ((uint8_t *)mdec->input)[1] == 0xD8)
        {
            log_info("MDEC: Detected JPEG data, using JPEG decoder");

            mdec->output = g_mdec_output_buffer;

            // Calculate output dimensions (16x16 blocks for MDEC)
            {
                unsigned long output_width = 16;
                unsigned long output_height = 16;

                int32_t result = mdec_jpeg_decode((uint8_t *)mdec->input, mdec->input_size,
                                                  mdec->output, output_width, output_height,
                                                  mdec->output_depth);

                if (result == 0)
                {
                    mdec->output_words_remaining = block_size >> 2;
                    mdec->output_empty = 0;
                    mdec->output_index = 0;
                    return;
                }
                ...
                {
                    log_warn("MDEC: JPEG decode failed, falling back to traditional MDEC decode");
                }
            }
        }

        // Traditional MDEC decoding (existing code)
        ...
        if (max_output_size > 1024 * 1024)
        { // Log if > 1MB
            log_info("MDEC: Using static buffer for %lu bytes (%.1f MB), input_size=%zu, estimated_blocks=%lu",
                     max_output_size, (float)max_output_size / (1024 * 1024), mdec->input_size, estimated_blocks);
        }
...
{
    PRINTF("Unhandled 16-bit MDEC read offset=%lu\n", offset);
...
{
    PRINTF("Unhandled 8-bit MDEC read offset=%lu\n", offset);
...
{
    PRINTF("Unhandled 16-bit MDEC write offset=%lu, value=%04x\n", offset, value);
}

void psx_mdec_write8(psx_mdec_t *mdec, uint32_t offset, uint8_t value)
{
    PRINTF("Unhandled 8-bit MDEC write offset=%lu, value=%02x\n", offset, value);
}
