...
psx_cdrom_t *psx_cdrom_create(void)
{
    ...
    {
        PRINTF("[CDROM] CDROM creation failed - malloc returned NULL\r\n");
    }
    return cdrom;
}
...
void psx_cdrom_init(psx_cdrom_t *cdrom, psx_ic_t *ic)
{
    ...
    PRINTF("[CDROM] Creating queues...\r\n");
    cdrom->data = queue_create();
    cdrom->response = queue_create();
    cdrom->parameters = queue_create();
    cdrom->ic = ic;

    if (cdrom->data && cdrom->response && cdrom->parameters)
    {
        PRINTF("[CDROM] Queues created successfully\r\n");
    }
    else
    {
        PRINTF("[CDROM] Queue creation failed\r\n");
    }
    ...
    PRINTF("[CDROM] CDROM initialization complete\r\n");
...
void psx_cdrom_set_version(psx_cdrom_t *cdrom, int32_t version)
{
    cdrom->version = version;
}

void psx_cdrom_set_region(psx_cdrom_t *cdrom, int32_t region)
{
    cdrom->region = region;
}

int32_t psx_cdrom_open(psx_cdrom_t *cdrom, const char *path)
{
    PRINTF("[CDROM] Opening disc: %s\r\n", path ? path : "NULL");

    if (!path)
    {
        PRINTF("[CDROM] Error: Path is NULL\r\n");
        return 1;
    }

    PRINTF("[CDROM] Resetting CDROM...\r\n");
    cdrom_cmd_reset(cdrom);

    PRINTF("[CDROM] Creating disc instance...\r\n");
    cdrom->disc = psx_disc_create();
    if (!cdrom->disc)
    {
        PRINTF("[CDROM] Error: Failed to create disc instance\r\n");
        return 0;
    }

    PRINTF("[CDROM] Opening disc file: %s\r\n", path);
    cdrom->disc_type = psx_disc_open(cdrom->disc, path);

    if (cdrom->disc_type == CDT_ERROR)
    {
        PRINTF("[CDROM] Error: Failed to open disc (type=CDT_ERROR)\r\n");
        psx_cdrom_close(cdrom);
        return 0;
    }

    PRINTF("[CDROM] Disc opened successfully, type: %d\r\n", cdrom->disc_type);
    return 1;
}
...
void cdrom_set_int(psx_cdrom_t *cdrom, int32_t n)
{
    cdrom->ifr = n;
}

int32_t cdrom_get_read_delay(psx_cdrom_t *cdrom)
{
    return (cdrom->mode & MODE_SPEED) ? CD_DELAY_READ_DS : CD_DELAY_READ_SS;
}

int32_t cdrom_get_pause_delay(psx_cdrom_t *cdrom)
{
...
int32_t cdrom_get_seek_delay(psx_cdrom_t *cdrom, int32_t ts)
{
    int32_t delay = cdrom->pending_speed_switch_delay;
...
void cdrom_handle_read(psx_cdrom_t *cdrom)
{
    ...
    int32_t ts = psx_disc_query(cdrom->disc, cdrom->lba);
    ...
    int32_t size_bit = cdrom->mode & MODE_SECTOR_SIZE;
    ...
    /* Convert to debug level - this is normal CDROM sector reading */
    log_debug("CDROM sector read: size=%x off=%u lba=%ld data=%02x:%02x:%02x:%02x delay=%u",
...
{
    int32_t data_empty = queue_is_empty(cdrom->data) || !cdrom->data_req;
...
uint8_t cdrom_read_data(psx_cdrom_t *cdrom)
{
    if (!cdrom->data_req)
    {
        log_debug("CDROM data read blocked: data_req=0");
        return 0;
    }

    /* Use debug level for CDROM data read - this is normal operation */
    log_debug("CDROM data read: pos=%x/%x data=%02x |%c|",
              cdrom->data->read_index,
              cdrom->data->write_index,
...
void cdrom_write_cmd(psx_cdrom_t *cdrom, uint8_t data)
{
    ...
    /* Convert CDROM command logging to debug level */
    log_debug("CDROM cmd: %-16s (%02x)", cdrom_cmd_names[data], data);
    ...
    char param_str[64] = "";
    for (int32_t i = 0; i < cdrom->parameters->write_index && i < 8; i++)
    {
        char temp[8];
        snprintf(temp, sizeof(temp), "%02x ", cdrom->parameters->buf[i]);
        strcat(param_str, temp);
    }
    if (param_str[0])
    {
        log_debug("CDROM params: %s", param_str);
    }
...
void cdrom_write_req(psx_cdrom_t *cdrom, uint8_t data)
{
    int32_t old_data_req = cdrom->data_req;
    cdrom->data_req = data & 0x80;

    if (cdrom->data_req != old_data_req)
    {
        log_debug("CDROM data_req changed: %d -> %d (data=0x%02x)",
                  old_data_req ? 1 : 0, cdrom->data_req ? 1 : 0, data);
    }
}
