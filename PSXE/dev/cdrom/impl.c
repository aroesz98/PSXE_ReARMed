...
{
    int32_t m = queue_pop(cdrom->parameters);
    int32_t s = queue_pop(cdrom->parameters);
    int32_t f = queue_pop(cdrom->parameters);
...
void cdrom_cmd_play(psx_cdrom_t *cdrom)
{
    ...
    {
        int32_t track = BTOI(queue_pop(cdrom->parameters));
    ...
    int32_t mm = cdrom->lba / (60 * 75);
    int32_t ss = (cdrom->lba % (60 * 75)) / 75;
    int32_t ff = (cdrom->lba % (60 * 75)) % 75;

    PRINTF("play song at lba=%08lx %02u:%02u:%02u track=%d\r\n", cdrom->lba, mm, ss, ff, track);
...
void cdrom_cmd_readn(psx_cdrom_t *cdrom)
{
    ...
    // Preload sector
    int32_t ts = psx_disc_read(cdrom->disc, cdrom->lba, cdrom->data->buf);
...
void cdrom_cmd_setmode(psx_cdrom_t *cdrom)
{
    ...
    int32_t prev_speed = cdrom->mode & MODE_SPEED;
...
{
    int32_t lba = cdrom->lba;

    int32_t track = psx_disc_get_track_number(cdrom->disc, lba);
    int32_t track_lba = psx_disc_get_track_lba(cdrom->disc, track);
    ...
    int32_t rmm = diff / (60 * 75);
    int32_t rss = (diff % (60 * 75)) / 75;
    int32_t rff = (diff % (60 * 75)) % 75;
    int32_t amm = lba / (60 * 75);
    int32_t ass = (lba % (60 * 75)) / 75;
    int32_t aff = (lba % (60 * 75)) % 75;

    int32_t index = psx_disc_query(cdrom->disc, lba) != TS_PREGAP;
...
void cdrom_cmd_gettn(psx_cdrom_t *cdrom)
{
    ...
    int32_t tn = psx_disc_get_track_count(cdrom->disc);
...
{
    int32_t bcd = queue_pop(cdrom->parameters);
    ...
    int32_t track = BTOI(bcd);
    int32_t f = psx_disc_get_track_lba(cdrom->disc, track);
    ...
    int32_t mm = f / (60 * 75);
    int32_t ss = (f % (60 * 75)) / 75;

    // PRINTF("gettd: track %u lba=%08lx (%u) %02u:%02u:%02u\n",
    //     track,
    //     f, f,
    //     mm, ss, ff
    // );
...
void cdrom_cmd_seekl(psx_cdrom_t *cdrom)
{
    ...
    {
        int32_t ts = psx_disc_query(cdrom->disc, cdrom->pending_lba);
...
{
    int32_t subf = queue_pop(cdrom->parameters);
