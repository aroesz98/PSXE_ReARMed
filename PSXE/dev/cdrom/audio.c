...
static const int32_t pos_adpcm_table[] = {
    0, +60, +115, +98, +122};
...
void cdrom_decode_xa_block(psx_cdrom_t *cdrom, int32_t idx, int32_t blk, int32_t nib, int16_t *buf, int16_t *h)
{
    int32_t shift = 12 - (cdrom->xa_buf[idx + 4 + blk * 2 + nib] & 0x0F);
    int32_t filter = (cdrom->xa_buf[idx + 4 + blk * 2 + nib] & 0x30) >> 4;
...
int32_t cdrom_fetch_xa_sector(psx_cdrom_t *cdrom)
{
    while (1)
    {
        int32_t ts = psx_disc_read(cdrom->disc, cdrom->xa_lba, cdrom->xa_buf);
        ...
        int32_t file_eq = cdrom->xa_buf[0x10] == cdrom->xa_file;
        int32_t channel_eq = cdrom->xa_buf[0x11] == cdrom->xa_channel;
...
int32_t cdrom_get_xa_samples(psx_cdrom_t *cdrom, void *buf, uint32_t size)
{
    ...
    {
        int32_t stereo = (cdrom->xa_buf[0x13] & 1) == 1;
...
int32_t cdrom_reload_cdda_buffer(psx_cdrom_t *cdrom, void *buf, uint32_t size)
{
    int32_t ts = psx_disc_read(cdrom->disc, cdrom->lba, cdrom->cdda_buf);
...
void cdrom_send_report_irq(psx_cdrom_t *cdrom)
{
    ...
    int32_t track = psx_disc_get_track_number(cdrom->disc, cdrom->lba);
    ...
    int32_t relative = (cdrom->cdda_sectors_played & 0x10) != 0;
